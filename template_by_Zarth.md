...
{  
    ...
    {  
        ...
        for(int j=i+1; j<len; j++)  
            if(str[j] < str[i]) tmp++;  
        ans += tmp * fact[len-i-1];  //f[]为阶乘  
    }  
    return ans;  //返回该字符串是全排列中第几大，从1开始  
}
//康托展开逆运算  
void KangtoInv(LL n,LL m)  
{  
    n--;  
    vector<int> v;  
    vector<int> a;  
...
// M=m1*m2*m3*...*Mk
// Mi=M/mi
// x ≡ (a1*M1*inv(M1,m1) + ... + ak*Mk*inv(Mk,mk)) mod M
...
// 莫比乌斯反演
// F(n)=sum(f(d)) (d|n) <==> f(n)=sum(u(n/d)F(d))  (d|n)
// F(n)=sum(f(d)) (n|d) <==> f(n)=sum(u(d/n)F(d))  (n|d)  （第二种形式）
